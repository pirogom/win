...
func FindWindowEx(hWndParent HWND, hWndChildAfter HWND, lpszClass *uint16, lpszWindow *uint16) HWND {
...
func GetWindowText(hWnd HWND, lpString *uint16, nMaxCount int32) int {
	ret, _, _ := syscall.Syscall6(getWindowText.Addr(), 3,
		uintptr(hWnd),
		uintptr(unsafe.Pointer(lpString)),
		uintptr(nMaxCount),
		0, 0, 0)
